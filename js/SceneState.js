...
const SceneState = {
	...
	scenes: {
		...
		[SCENE.ASSIST]: new AssistScene2(),
		[SCENE.SLIDER]: new ModalSlider(),
		[SCENE.LEVEL_INTRO]: new LevelIntroScene(),
	...
	run: function(deltaTime, previousState) {
		...
		if (scene) {
			...
			if (sound.getIsMuted()) {
				JPFont.printTextAt(getLocalizedStringForKey(STRINGS_KEY.Muted), {x:760, y: 10}, TextAlignment.Left, 14);
			}
